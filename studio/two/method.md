...
Declaration Jobs with matching run records link directly to output. Their adjacent status describes the latest recorded attempt; an unclosed record does not establish a running process. Multiple attempts are named without collapsing the individual timeline rows. Tranches and unmatched declarations remain noninteractive, and the canvas graph remains a view of inferred precedence.

Log detail puts process output first and annotates its source on the log surface. An unclosed attempt leads with the browser's Pod observation; a closed attempt leads with stored output. Both sources remain visible when available. For a sole recorded attempt, the containers foreign key associates stored output with that attempt. With multiple attempts, the latest-only address cannot establish which attempt supplied the returned text. An observed process exit remains separate from the recorded exit status. The server selects a current Pod by phase and creation time, without matching it to a recorded attempt or returning its name to the browser.

An observation already in progress survives a closing Job write, including lines received before an exit, disconnect, or error. Saved observations are scoped to the latest recorded attempt's start timestamp so a retry cannot inherit an earlier observation. Pending harvest is routine while an attempt is unclosed; stored reads continue briefly after completion and stop for settled records. Run summaries describe each Job identity's latest attempt while the timeline preserves every attempt.

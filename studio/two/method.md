...
Log detail puts process output first and annotates its source on the log surface. An open tail is a Pod observation made by this browser. Stored output is the latest text addressable by run, Job, and container identity, and it may exist even while the Job row remains unclosed. Both sources are shown when both exist. Because the current endpoint does not accept an attempt timestamp, the interface does not promise that retrieved output belongs to a particular historical attempt. An observed process exit remains separate from the recorded exit status, and the selected Pod is not named because the tail response does not identify it.
